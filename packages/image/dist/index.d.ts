import { Vector2D, Point, Segment, Polygon } from '@hycord/math';
import { Canvas as Canvas$1 } from '@napi-rs/canvas';
...
type OmitMatrixMethod = {
  [K in keyof DOMMatrix]: DOMMatrix[K] extends (...args: unknown[]) => unknown
    ? never
    : K;
};
...
interface AvifConfig {
  ...
  speed?: number;
  /** How munknown threads should be used (0 = match core count) */
  threads?: number;
...
declare enum ChromaSubsampling {
   ...
   * Note that "4:4:4" may instead be wrongly referring to R'G'B' color space, which implicitly also does not have unknown chroma subsampling (except in JPEG R'G'B' can be subsampled).
   ...
   * Munknown high-end digital video formats and interfaces use this scheme:
...
declare class Viewport {
    ...
    get width(): number;
    get height(): number;
    shape(draw: (ctx: SKRSContext2D) => void): Promise<void>;
    set center(newCenter: Vector2D);
    get center(): Vector2D;
    set zoom(newZoom: number);
    get zoom(): number;
    render(): Promise<Buffer>;
    drawPoint(point: Point, { fillColor, lineWidth, strokeColor, outline, radius, text, textFillColor, textStrokeColor, }?: DrawConfiguration): void;
    drawSegment(segment: Segment, { lineWidth, strokeColor, }?: DrawConfiguration): void;
    drawPolygon(polygon: Polygon, { fillColor, lineWidth, }?: DrawConfiguration): void;
}

export { type SKRSContext2D, Viewport };
